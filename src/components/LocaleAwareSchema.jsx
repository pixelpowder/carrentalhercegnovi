...
const BASE_AUTO_RENTAL = {
  ...
  "address": {
    "@type": "PostalAddress",
    "streetAddress": "Trg Mičela Komnena",
...
export default function LocaleAwareSchema({ lang = 'en', isHomepage = true }) {
  ...
  const vehicleList = (config.cars || []).map((car, i) => ({
    ...
    "item": {
      ...
      "description": (() => {
        const tpl = pick(t, 'schema.carDescription') || pick(translations.en, 'schema.carDescription') || '{category} rental, {transmission}, {fuel}, {seats} seats';
        return tpl
